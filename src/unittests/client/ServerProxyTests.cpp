...
ServerProxy makeProxy(Client *client, deskflow::IStream *stream, IEventQueue *events)
{
  return ServerProxy(client, stream, events, false, false, false, false, false);
}
...
void ServerProxyTests::handleKeepAliveAlarm_timeout_queuesDisconnectRequest()
{
  ...
  FakeStream stream;
  auto proxy = makeProxy(undereferenceableClient(), &stream, &events);
...
void ServerProxyTests::parseHandshakeMessage_protocolError_queuesRefusalRequest()
{
  RecordingEventQueue events;
  FakeStream stream;
  TestServerProxy proxy(undereferenceableClient(), &stream, &events, false, false, false, false, false);
